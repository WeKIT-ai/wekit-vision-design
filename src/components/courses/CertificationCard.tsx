...
const CertificationCard = ({ course }: CertificationCardProps) => {
  return (
    <Card className="group border border-border/60 bg-card hover:shadow-lg transition-all duration-300 overflow-hidden">
      <CardContent className="p-8 space-y-5">
        {/* Badge placeholder */}
        <div className="flex items-center justify-between">
          <div className="w-12 h-12 bg-wekit-soft flex items-center justify-center">
        ...
        <div className="flex flex-wrap gap-2">
          {course.tags.map((tag) => (
            <span
              key={tag}
              className="px-3 py-1 text-xs font-medium bg-wekit-tag text-wekit-tag-text"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <Button variant="outline" size="icon" className="border-border" title="Download Syllabus (Coming Soon)" disabled>
