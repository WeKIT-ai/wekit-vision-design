...
const WaitlistHero = () => {
  const { name, setName, email, setEmail, isLoading, handleSubmit, policyAccepted, setPolicyAccepted, showPolicyError, setShowPolicyError } = useWaitlistForm();
    ...
    <section className="min-h-screen flex items-center pt-20 pb-16 px-4 sm:px-6 lg:px-8 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto w-full grid lg:grid-cols-5 gap-12 items-center">
        <div className="lg:col-span-3">
          ...
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="font-montserrat font-bold text-4xl md:text-5xl lg:text-[56px] leading-tight text-wekit-black mb-6">
          ...
          <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="text-lg text-gray-600 mb-8 max-w-xl leading-relaxed">
          ...
          <motion.form initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} onSubmit={handleSubmit} className="flex flex-col gap-3 mb-6 max-w-xl">
            ...
            <PolicyAcceptance 
              ...
              showMinorNotice 
            />
            <Button type="submit" disabled={isLoading || !policyAccepted} className="h-12 px-8 rounded-lg bg-gradient-to-r from-wekit-blue to-wekit-zaffre hover:opacity-90 text-white font-montserrat font-semibold whitespace-nowrap">
              {isLoading ? 'Joining...' : 'Join Waitlist'}
            </Button>
          </motion.form>

          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.5 }} className="flex items-center gap-3">
        ...
        <div className="lg:col-span-2 relative hidden lg:block h-[500px]">
          ...
          {floatingCards.map((card, i) => (
            <motion.div key={i} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: card.delay }} className="absolute bg-white rounded-xl shadow-lg px-5 py-3 flex items-center gap-3 border border-gray-100" style={{ top: card.top, right: card.right }}>
              <span className="text-2xl">{card.emoji}</span>
              <span className="font-montserrat font-semibold text-sm text-wekit-black">{card.label}</span>
            </motion.div>
          ))}
          <motion.div animate={{ y: [0, -15, 0] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 bg-gradient-to-br from-wekit-blue to-wekit-green rounded-2xl opacity-20 blur-2xl" />
        </div>
      </div>
    </section>
  );
