    ...
    <section id="how-it-works" ref={ref} className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-[#F9FAFB] to-white">
      <div className="max-w-6xl mx-auto text-center">
        ...
        <div className="grid md:grid-cols-3 gap-8 relative">
          ...
          {steps.map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.2 + i * 0.15 }} className="relative">
              <div className="w-12 h-12 rounded-full bg-gradient-to-r from-wekit-blue to-wekit-zaffre text-white font-montserrat font-bold text-sm flex items-center justify-center mx-auto mb-6 relative z-10">{s.num}</div>
